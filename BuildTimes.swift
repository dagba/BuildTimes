...
func startBuild() {
	...
	if let index = buildData.firstIndex(where: { $0.date == dateKey }) {
        ...
        buildData.append(BuildData(date: dateKey, totalBuildTime: 0, lastStart: startDate, totalBuilds: 0, averageBuildTime: ""))
	}
	write(buildData)
}
...
func endBuild() {
	...
	if let index = buildData.firstIndex(where: { $0.date == dateKey }) {
        ...
        buildData[index].averageBuildTime = (buildData[index].totalBuildTime / Double(max(buildData[index].totalBuilds, 1))).asReadableTime()
	}
	write(buildData)
}
